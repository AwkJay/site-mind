...
## HexaFalls update — the framework rule is now scoped, not blanket

Everything above this line predates the HexaFalls phase and is still accurate for the pillars it
describes (Compliance, Commissioning, cost-risk, the impact model, schedule mitigation) — none of
that changed. One boundary DID change: **`backend/app/agents/copilot_agent.py` (plan §F2) now runs
LangGraph**, specifically and only for the Copilot's conversational edge (`POST
/api/copilot/chat` — multi-turn, multi-pillar routing + conversation memory via a LangGraph
checkpointer). The reasoning above for "why not a framework" still holds for every pillar's
*verdict* logic — it just no longer holds for a bounded, read-only Q&A router:

- The agent's tools are thin, READ-ONLY callers of already-existing, already-deterministic
  functions (`copilot._hybrid_retrieve`, `Corpus.query`, `compliance.evaluate` for reading —
  not computing — NCRs, `schedule.risks()`, `supply_chain.risks()/alerts()`). None of them can
  produce a new verdict; `copilot_agent.py` doesn't import `checks.py` or `rule_eval.py` at all.
- It replaces a genuine limitation of the old single-shot Copilot (no memory, no cross-pillar
  routing in one answer) — the same "is this the right depth for the task" test applied to the
  mitigation agents above, applied here to conversation, not verdicts.
- The boundary is enforced by what the module can even import, not by convention: there is no
  code path from `copilot_agent.py` into the deterministic core, so this is a hard architectural
  guarantee, not a "please don't."
- Gated on `COPILOT_AGENT_ENABLED` + a present `GEMINI_API_KEY`, imported lazily inside the module
  — a keyless/flag-off boot never even attempts to import `langgraph`/`langchain-google-genai`.
