...
# This script lives in docs/ (gitignored — it is tooling, not part of the public
# repo), so resolve the repo root via git rather than the script's own directory.
SRC="$(git -C "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" rev-parse --show-toplevel)"
