...
expected value was checked against before being hardcoded here):
  ...
  1. list_corpora        — real corpus doc/chunk counts match the state
                            ...
                            (structural_standard_codes, renamed from manak_structural/codebook_structural per
...
#     structural_standard_codes (renamed from manak_structural/codebook_structural, docs/codebook_changes.md
...
MANAK_SOURCE_FILE = (
    ...
    / "data" / "structural_corpus" / "is1893_part1_2016" / "irc.gov.in.1893.2016.md"
)
CLAUSES_JSON_FILE = (
...
async def _cases_list_corpora(session: ClientSession) -> list[dict]:
    ...
    cases.append(
        _new_case(
            "list_corpora_structural_standard_codes_counts",
            "list_corpora",
            True,
            ("structural_standard_codes" in text and "documents: 17" in text and "chunks: 6206" in text),
        )
    )
    cases.append(
        _new_case(
...
async def _cases_search_standards(session: ClientSession) -> list[dict]:
    ...
    # heading "3.10 Importance Factor (I)", scoped to structural_standard_codes.
    is_error, text = await _call_tool(
        session,
        "search_standards",
        {
            "query": "importance factor for data centre building seismic design",
            "corpus_name": "structural_standard_codes",
            "k": 3,
        },
    )
    ...
    for i, q in enumerate(["best vacation destinations in Europe", "recipe for chocolate chip cookies"], 1):
        is_error, text = await _call_tool(
            session, "search_standards", {"query": q, "corpus_name": "structural_standard_codes", "k": 3}
        )
...
async def _cases_get_clause(session: ClientSession) -> list[dict]:
    ...
    # Filesystem corpus #1 (structural_standard_codes): raw_text is a literal
