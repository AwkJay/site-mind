...
existing, already-real text sources, purely as a parallel READ path:
  ...
  1. `structural_standard_codes` (renamed from `manak_structural` ->
     `codebook_structural` -> `structural_standard_codes` —
     docs/codebook_changes.md item 2, 2026-07-12, then the 2026-07-25
     follow-up rename that dropped the internal "codebook" codename too) —
     every `.md` file under manak-dev's own clause library
     (`STRUCTURAL_LIB_DIR` below), indexed with THIS package's own chunker
     (never manak's `chunker.py`/`indexer.py` — zero code coupling to that
     separate, independently-versioned service). Provenance tag
     `"codebook_verified"` (renamed from `"manak_indexed"` — item 1).
...
# "structural_standard_codes" (docs/codebook_changes.md item 2, 2026-07-12;
# then 2026-07-25) — this is what list_corpora/search_standards actually
# return to any live caller.
...
# the `logger.warning` in `_structural_md_files()` below). Per
...
# `lib/<doc_id>/<file>.md` nesting `_structural_md_files()`/
# `build_structural_standard_codes_corpus()` depend on. manak-dev/ itself
# was never modified — read-only source, copy-in only. Computed relative to
