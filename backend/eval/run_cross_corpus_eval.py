"""Held-out eval for Phase 3b — the two read-only, filesystem-backed
retrieval corpora added on top of the Phase 3 retrieval package:

  - `structural_standard_codes`            (17 real `.md` files under manak-dev/lib/)
...
Five groups:
  ...
  (e) manak_untouched          — building `structural_standard_codes` never wrote,
      moved, or deleted anything under manak-dev/ (mtimes of every file this
      eval reads are unchanged after the build).

NOTE: this eval genuinely builds `structural_standard_codes` from ~6,200 real chunks
...
Run: `python -m eval.run_cross_corpus_eval` from `backend/` (venv active).
Writes `backend/eval/cross_corpus_report.json` in the same
...
from app.retrieval.filesystem_corpora import (  # noqa: E402
    STRUCTURAL_CORPUS_NAME,
    STRUCTURAL_LIB_DIR,
    SITEMIND_CORPUS_NAME,
    SITEMIND_STANDARDS_DIR,
    SITEMIND_STANDARDS_FILES,
    _structural_md_files,
    build_structural_standard_codes_corpus,
...
def main() -> None:
    ...
    manak_files = _structural_md_files()
    before = _file_stat_snapshot(manak_files)

    manak = build_structural_standard_codes_corpus()
    sm = build_sitemind_standards_corpus()

    after = _file_stat_snapshot(_structural_md_files())
    ...
    print(
        f"  structural_standard_codes: {manak.chunk_count} chunks / {len(manak.document_ids)} docs | "
