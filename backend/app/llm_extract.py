...
# Interpretive parameter types (plan §B2, "computed_draft" tier): no checks.py
# rule exists for these YET — that's exactly why they exist here rather than in
# CHECKABLE_TYPES — but a real standards clause almost certainly governs them.
# Widened HERE (the type allowlist) only, never the substring/value gate below:
# an EXTRACTABLE_TYPES item still needs a verbatim quote like everything else.
# These route through agents/compliance.py's per-param loop to
# `_computed_draft_finding` (an LLM reads a rule out of a retrieved clause;
# `rule_eval.py` computes it) rather than straight to a checks.py verdict.
EXTRACTABLE_TYPES: dict[str, str] = {
    "min_grade_of_concrete": "the minimum grade of concrete specified/required, as the number after 'M' (e.g. M25 -> 25), in MPa.",
    "bar_spacing_mm": "centre-to-centre spacing between reinforcement bars, in mm.",
    "min_bearing_length_mm": "minimum bearing/seating length of a beam or slab on its support, in mm.",
}
...
def verify_spans(raw_items: list[dict], text: str) -> tuple[list[ExtractedParam], list[Abstention]]:
    ...
    for item in raw_items or []:
        ...
        # (d) type must be recognised — either a certified checks.py type, or an
        # EXTRACTABLE_TYPES interpretive type routed to the computed_draft tier.
        if param_type not in CHECKABLE_TYPES and param_type not in EXTRACTABLE_TYPES:
            drop(param_type, f"'{param_type}' is not a recognised parameter type — dropped.")
            continue
        ...
        found.append(
            ExtractedParam(
                ...
                value=value,
                # EXTRACTABLE_TYPES has no hand-curated unit table (that's the point —
                # nothing has vetted these types yet), so fall back to whatever unit
                # string the model itself reported.
                unit=_UNIT_BY_TYPE.get(param_type) or str(item.get("unit") or "").strip(),
...
def _build_prompt(text: str) -> tuple[str, str]:
    ...
    extractable_lines = "\n".join(f"  - {k}: {v}" for k, v in EXTRACTABLE_TYPES.items())
    system = (
        ...
        f"{enum_lines}\n"
        "  1b. OR one of these (no pre-vetted rule exists yet, but still worth flagging):\n"
        f"{extractable_lines}\n"
