...
Dense search itself goes through `vector_store.py` (plan §C — Actian VectorAI
DB): `RETRIEVAL_VECTOR_STORE=numpy` (default) keeps the original exhaustive
`matrix @ q` ranking every eval here is calibrated against, byte-identical;
`=actian` routes to a real offline Actian VectorAI DB instead, proven correct
by its own parity eval, never by these numpy-calibrated ones. An unreachable
Actian container falls back to numpy with a logged warning rather than
crashing retrieval.
"""
...
# Candidate pool size for an Actian ANN search feeding the RRF fusion below —
# a real vector index legitimately doesn't return every vector (unlike the
# numpy path, which is exhaustive by construction and must stay byte-identical
# for the numpy-calibrated evals). Not tuned against any eval; just generous
# enough to give BM25 fusion a reasonable dense candidate set.
_ACTIAN_CANDIDATE_POOL = 200
...
class Corpus:
    ...
    def _rebuild_indices(self) -> None:
        ...
        if config.RETRIEVAL_VECTOR_STORE == "actian":
            try:
                store = vector_store.actian_store(config.ACTIAN_URL)
                store.ensure_collection(self.name, dim=self._matrix.shape[1])
                ids = list(range(len(self.chunks)))
                store.upsert(self.name, ids, self._matrix, [{"chunk_index": i} for i in ids])
            except Exception:
                logger.warning(
                    "Corpus %r: Actian upsert failed; query() will fall back to numpy.",
                    self.name,
                    exc_info=True,
                )

    def _dense_search(self, q: np.ndarray) -> list[tuple[int, float]]:
        if config.RETRIEVAL_VECTOR_STORE == "actian":
            try:
                return vector_store.actian_store(config.ACTIAN_URL).search(
                    self.name, q, limit=min(len(self.chunks), _ACTIAN_CANDIDATE_POOL)
                )
            except Exception:
                logger.warning(
                    "Corpus %r: Actian search failed; falling back to numpy.", self.name, exc_info=True
                )
        # Exhaustive by construction (limit=len(chunks)) — the exact ranking
        # every numpy-calibrated eval asserts against. Never truncated.
        return vector_store.numpy_store().search(self._matrix, q, limit=len(self.chunks))
    ...
    def query(self, text: str, k: int = 4) -> list[dict]:
        ...
        if not dense_hits or dense_hits[0][1] < RETRIEVAL_FLOOR:
            return []  # abstain — even the single best dense match is too weak

        dense_order = [idx for idx, _score in dense_hits]
        score_by_idx = dict(dense_hits)
        ...
        for i in fused[:k]:
            ...
            # Every index has a real numpy score available (embed() always
            # runs regardless of vector-store backend); fall back to it for an
            # index outside Actian's bounded candidate pool.
            chunk["score"] = float(score_by_idx.get(i, self._matrix[i] @ q))
