...
class ActianVectorStore:
    ...
    def _connect(self):
        if self._client is None:
            # NOTE: the PyPI package is named `actian-vectorai-client` but its
            # importable module is `actian_vectorai` (confirmed against the
            # real installed package — the plan's guessed name was the pip
            # name, not the import name).
            from actian_vectorai import VectorAIClient  # lazy: optional dependency

            client = VectorAIClient(self.url)
            client.connect()  # must be called explicitly before any RPC (confirmed against the real client)
            client.health_check()
            self._client = client
        return self._client
    ...
    def ensure_collection(self, name: str, dim: int = 384) -> None:
        ...
        from actian_vectorai import Distance, VectorParams  # lazy
    ...
    def upsert(self, name: str, ids: list[int], vectors: np.ndarray, payloads: list[dict]) -> None:
        from actian_vectorai import PointStruct  # lazy
