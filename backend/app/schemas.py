...
class ExtractedRule(BaseModel):
    """A rule the LLM *read* out of one real, retrieved clause — never invented, never
    computed by the LLM. `rule_eval.evaluate()` is the only thing that turns this into a
    PASS/FAIL/NOT_CHECKABLE verdict; the LLM's role stops at perceiving the rule shape.
    `clause_phrase` must be a verbatim substring of the citation's clause text (checked by
    the caller before this rule is ever evaluated) — never fabricated."""
    kind: Literal["compare", "range", "min_of", "max_of", "table_lookup", "formula", "none"]
...
class NCR(BaseModel):
    ...
    # confirm the reading; "unresolved" = honestly flagged, no rule could be established.
    verdict_tier: Literal["certified", "computed_draft", "unresolved"] = "certified"
    extracted_rule: Optional[ExtractedRule] = None
    # The exact detail string rule_eval.evaluate() computed (e.g. "value 20.0 MPa
...
class CoverageStat(BaseModel):
    ...
    computed_draft_count: int = 0  # NCRs from the computed_draft tier (rule_eval, plan §B2)
    unresolved_count: int = 0      # params flagged unresolved (no rule established), never silently dropped
...
# --------------------------------------------------------------------------- #
# Audit ledger (plan §D) — the append-only project memory flat-file
# recomputation never had. Every finalized compliance decision is recorded
# once (idempotent via content_hash); nothing here is ever updated or deleted
# except the `solana` sub-doc (workstream E's on-chain notarization status).
# --------------------------------------------------------------------------- #
class SolanaAnchor(BaseModel):
    status: Literal["pending", "anchored", "error", "disabled"] = "pending"
...
class AuditEvent(BaseModel):
    """One immutable audit record. `payload`/`hashed_fields`/`content_hash`
    are set once at insert and never changed; `solana` is the only field
    mutated afterward. `hashed_fields` is the (possibly narrower) subset of
    `payload` that `content_hash` was actually computed over — see
    audit.py's `record_event` docstring for why they can differ."""
    id: str
    seq: int
    created_at: float
    pillar: str
    ...
    hashed_fields: dict
    content_hash: str
    solana: SolanaAnchor = SolanaAnchor()
