...
export default function KnowledgeBasePage() {
  ...
  const [vectorStore, setVectorStore] = useState<string>("numpy");
  ...
  useEffect(() => {
    refreshCorpora();
    getHealth().then(({ data }) => setVectorStore(data.vector_store ?? "numpy"));
  }, [refreshCorpora]);
    ...
    <div>
      ...
      {availability === "available" && (
        <div className="-mt-2 mb-4">
          <Chip
            color={vectorStore === "actian" ? "var(--accent)" : "var(--text-lo)"}
            bg={vectorStore === "actian" ? "rgba(190,242,100,0.12)" : "rgba(159,176,191,0.10)"}
          >
            Vector store: {vectorStore === "actian" ? "Actian VectorAI (offline)" : "numpy (in-memory)"}
          </Chip>
        </div>
      )}
